...
- [x] **RCA** = SID終点までにアサインドALTへ到達する点(v6-151)。SID実距離は図の値を手入力。「地図に反映」で飛行ログの注記点(括弧の内数)になり印刷にも載る。TOD(降下開始点)は未実装
- [ ] **Stage 3(SIDの形)ができれば** SID実距離の手入力が不要になり、RCA の位置も経路上の正しい場所に置ける。まず立川(EDARR/OMIYA)・館山・横田から
