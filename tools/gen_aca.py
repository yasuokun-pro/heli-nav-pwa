...
EXTRA_POINTS = {
 ...
 'RJOE/ACA': {   # 明野。⚠ **座標が3点しか書かれておらず、その3点はどれも頂点ではない**。
   #   (A)344217N/1365729E は北限線の基準、(B)343722N/1365140E と
   #   (C)343140N/1364148E は**弧の中心**(図では中心から破線が伸びていて、
   #   14NM/21NM/20NM の寸法が付いている)。頂点は全部、弧と線の交点で作る。
   # ⚠ 図に打たれている黒丸は座標とずれている(Bの丸は真位置から1.9NM西)。
   #   丸の位置ではなく**弧そのものに円を当てはめて**中心を決めること。
   # ⚠ 南東側は**浜松ACAと接している**。(4)(5)(6)は浜松ACAの(20)(5)(6)と同じ点で、
   #   図の「60NM」は LHT 60NM(=浜松の(6))を指す。明野の図は隅ほど作図が粗く、
   #   (5)の位置は図から読むと3NMずれる(線とCBE33弧の交角が12°しかないため)。
   #   **浜松の座標を優先する**こと
   1:(34.828549,136.522308),  # 344942.8N/1363120.3E CBE14 ∩ C20(三日月の北の角)
   2:(34.637247,136.894843),  # 343814.1N/1365341.4E CBE14 ∩ 北限線
   3:(34.642364,137.326176),  # 343832.5N/1371934.2E 北限線 ∩ LHT20
   4:(34.566410,137.369940),  # LHT20 ∩ CBE33      = 浜松ACAの(20)
   5:(34.392500,137.158333),  # 342333N/1370930E    = 浜松ACAの(5) CBE33@147.8°
   6:(34.104444,136.782222),  # 340616N/1364656E    = 浜松ACAの(6) LHT60NM(図の「60NM」)
   7:(34.237901,136.496979),  # 341416.4N/1362949.1E (6)から引いた接線のC20接点
   8:(34.741541,136.557835),  # 344429.5N/1363328.2E CBE14 ∩ 2500/4000界
   9:(34.593924,136.580156),  # 343538.1N/1363448.6E 2500/4000界のB14接点
  10:(34.444159,137.043283),  # 342639.0N/1370235.8E 北東の2500/4000界のB14接点
  11:(34.309041,137.049361),  # 341832.5N/1370257.7E B21 ∩ (5)-(6)線
  12:(34.768579,136.416988),  # 344606.9N/1362501.2E C20 ∩ 4000/5000界
  13:(34.588549,136.438455),  # 343518.8N/1362618.4E 4000/5000界のB21接点
 },
}
...
# 明野ACA。⚠ **AD 2.17の本文は「SEE RJOE attached chart」だけ**で、図にも
#   番号付きの座標表が無い。図に書かれているのは弧の中心と半径の寸法だけなので、
#   頂点は弧と線の交点として計算する(→ EXTRA_POINTS['RJOE/ACA'])。
#   使う弧: CBE 14/33NM、LHT 20NM、(B)343722N/1365140E の 14/21NM、
#           (C)343140N/1364148E の 20NM。
#   直線は3種類:
#     ・北限 = 「344217N/1365729E から XAC(大島VOR)へ引いた線の4NM南」。
#       図の破線と「4NM」の寸法がこれ。ほぼ東西(真方位89.1°)になる
#     ・南東 = 浜松ACAの(5)-(6)線(図では「046°34'T fm KEC」の破線と「5NM」で
#       描かれているが、**破線の作図が1.5°ほど狂っている**ので浜松の座標で引く)
#     ・南西 = (6)から C 20NM円へ引いた接線
#   区画の内部境界(2500/4000界と4000/5000界)は図にしか無く、**いずれも
#   Bの円への接線**になっている(接することは実測で確かめた: 2500/4000界は
#   B14に、4000/5000界はB21に、北東の界もB14に接する)。位置は図から起こした近似。
# ⚠ 単独ラベル 2500/4000/5000 は**下限**。根拠は2つ:
#   1. 明野飛行場自身が2500の区画の中にあり、AKENO CTRが「2500ft以下」。
#      2500が上限だとCTRと二重になって進入管制にならない
#   2. 隣の浜松ACAも4000/6000が下限で、明野の4000区画は
#      **LHT20弧・CBE33弧を挟んで浜松の4000区画と接している**
AK_B = (34.6227778, 136.8611111)   # 343722N/1365140E 14/21NM弧の中心
AK_C = (34.5277778, 136.6966667)   # 343140N/1364148E 20NM弧の中心
SPEC['RJOE/ACA'] = dict(
   jp='明野進入管制区', n='AKENO ACA',
   eff_note='図に上限の記載なし。区画の内部境界は図から起こした近似',
   outer=[{'arcp':(1,8),'c':CBE}, {'arcp':(8,2),'c':CBE}, 3, {'arcp':(3,4),'c':LHT},
          {'arcp':(4,5),'c':CBE}, 6, {'arcp':(7,12),'c':AK_C}, {'arcp':(12,1),'c':AK_C}],
   sub=[
     # 飛行場を含む内側。北は「XAC線の4NM南」、南はB14弧
     dict(n='2500', lo=2500, ring=[{'arcp':(8,2),'c':CBE}, 3, 10,
                                   {'arcp':(10,9),'c':AK_B}, 8]),
     # B14弧とB21弧に挟まれた帯。東は浜松ACAとの境(LHT20→CBE33→(5)-(6)線)
     dict(n='4000', lo=4000, ring=[{'arcp':(1,8),'c':CBE}, 9, {'arcp':(9,10),'c':AK_B}, 3,
                                   {'arcp':(3,4),'c':LHT}, {'arcp':(4,5),'c':CBE}, 11,
                                   {'arcp':(11,13),'c':AK_B}, 12, {'arcp':(12,1),'c':AK_C}]),
     # 西の三日月。外側はC20弧と(6)への接線
     dict(n='5000', lo=5000, ring=[13, {'arcp':(13,11),'c':AK_B}, 6, 7,
                                   {'arcp':(7,12),'c':AK_C}]),
   ])
...
def main():
    ...
    for fn, nm, jp, ic, why in (
            ...
            ('tca_naha.gen.json', 'NAHA TCA', '那覇ターミナルコントロールエリア', 'ROAH',
             '座標表8点でジオリファレンスできるが、放射線の向きは図の作図に従っている')):
