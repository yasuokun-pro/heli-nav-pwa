...
- **自衛隊・在日米軍施設**: AIPに載るのは飛行場だけなので、駐屯地・分屯地・演習場は
  **OpenStreetMap(Overpass API)** から取得 → `tools/gen_jsdf.py` → `jsdf.json`(278件)。
  種別 飛/駐/演/基、所属 陸/海/空/米/防。演習場は赤、他は所属で色分け。
  - **ODbLなので出典表示が必須**(レイヤーON時に地図クレジットへ自動追加・ポップアップにも明記)
  - OSMは有志データで**網羅性がない**(高遊原・八尾・徳島・対馬などが欠落)。
