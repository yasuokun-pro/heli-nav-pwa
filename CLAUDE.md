...
- **自衛隊・在日米軍施設**: AIPに載るのは飛行場だけなので、駐屯地・分屯地・演習場は
  外部から拾う。`tools/gen_jsdf.py` → `jsdf.json`(**320件**・107KB)。
  ...
  - それでも**網羅性は完全ではない**(高遊原分屯地はウィキペにも座標が無く欠落)。
    位置・範囲も保証されないので参考表示に留めること
...
- **民間訓練試験空域**: AIP **ENR 5.3.1**。`tools/gen_civ.py` → `civ.json`(**114区画**)。
  ...
  - 新幹線・河川・高速道路の中心線で定義された区画(KK 4-2〜4-8等)は座標が無く
    描けない。生成時に件数を出しているので把握しておくこと
